...
public class LeanPhotoSortrView extends View implements
		MultiTouchObjectCanvas<LeanPhotoSortrView.Img> {
	...
	public LeanPhotoSortrView(Context context) {
		this(context, null);
	}

	public LeanPhotoSortrView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public LeanPhotoSortrView(Context context, AttributeSet attrs, int defStyle) {
	...
	class Img {
		...
		private void initPath() {
			...
			case 0:
				...
				path.moveTo(0, LTY);
				path.lineTo(0, LTY + itemHgt - 100);
				path.lineTo(RTX, LTY + itemHgt + 100);
				path.lineTo(RTX, LTY);
				path.close();
				break;
			case 1:
				LTY = (itemHgt + border) * order - 100;
				RTX = display.widthPixels;
				path.moveTo(0, LTY);
				path.lineTo(0, LTY + itemHgt + 200);
				path.lineTo(RTX, LTY + itemHgt);
				path.lineTo(RTX, LTY + 200);
				path.close();
				break;
			case 2:
				LTY = (itemHgt + border) * order + 100;
